...
@router.get("/")
async def listar_categorias(db: Session = Depends(get_db), _claims: dict = Depends(get_current_subject)):
    cats = db.query(Categoria).order_by(Categoria.nombre).all()
    return {"status": "200", "total": len(cats), "data": [_serialize(c) for c in cats]}


@router.get("/{id}")
async def obtener_categoria(id: int, db: Session = Depends(get_db), _claims: dict = Depends(get_current_subject)):
...
@router.post("/")
async def crear_categoria(payload: CategoriaCreate, db: Session = Depends(get_db), _claims: dict = Depends(get_current_subject)):
...
async def actualizar_categoria(
    id: int, payload: CategoriaCreate, db: Session = Depends(get_db), _claims: dict = Depends(get_current_subject)
):
    c = db.query(Categoria).filter(Categoria.id == id).first()
    if not c:
        raise HTTPException(status_code=404, detail="Categoría no encontrada")
...
@router.delete("/{id}")
async def eliminar_categoria(id: int, db: Session = Depends(get_db), _claims: dict = Depends(get_current_subject)):
    c = db.query(Categoria).filter(Categoria.id == id).first()
    if not c:
        raise HTTPException(status_code=404, detail="Categoría no encontrada")
    db.delete(c)
